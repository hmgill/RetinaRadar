...
class RetinaRadarDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        # Get input size from config (default to 224 for most models)
        # For InceptionV3, set input_size to 299 in your config
        self.input_size = config["tl"]["model"].get("input_size", 224)
        ...
        self.train_transform = A.Compose([
            A.RandomResizedCrop(size=(self.input_size, self.input_size)),
        ...
        # Resize to slightly larger size, then center crop
        resize_size = int(self.input_size * 1.1)
